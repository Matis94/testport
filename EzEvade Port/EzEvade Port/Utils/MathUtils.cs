namespace EzEvade_Port.Utils
{
    ...
    class MathUtils
    {
        ...
        public static Vector2 RotateVector(Vector2 start, Vector2 end, float angle)
        {
            ...
            var ret = end;
            ret.X = (float) Math.Cos(angle) * (end.X - start.X) - (float) Math.Sin(angle) * (end.Y - start.Y) + start.X;
            ret.Y = (float) Math.Sin(angle) * (end.X - start.X) + (float) Math.Cos(angle) * (end.Y - start.Y) + start.Y;
            return ret;
        }
        ...
        public static Tuple<float, float> LineToLineIntersection(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4)
        {
            ...
            return Tuple.Create(((x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3)) / d, ((x2 - x1) * (y1 - y3) - (y2 - y1) * (x1 - x3)) / d);
        }
        ...
        public static float VectorMovementCollisionEx(Vector2 targetPos,
                                                      Vector2 targetDir,
                                                      float targetSpeed,
                                                      Vector2 sourcePos,
                                                      float projSpeed,
        ...
        {
            if (Math.Max(start.X, end.X) > point.X && point.X > Math.Min(start.X, end.X) && Math.Max(start.Y, end.Y) > point.Y && point.Y > Math.Min(start.Y, end.Y))
            {
                return true;
            }
        ...
        public static float GetCollisionDistanceEx(Vector2 Pa, Vector2 Va, float Ra, Vector2 Pb, Vector2 Vb, float Rb, out Vector2 PA, out Vector2 PB)
        {
            bool collision;
        ...
        public static float GetCollisionDistance(Vector2 Pa, Vector2 PaEnd, Vector2 Va, float Ra, Vector2 Pb, Vector2 PbEnd, Vector2 Vb, float Rb)
        {
            bool collision;
            var collisionTime = GetCollisionTime(Pa, Pb, Va, Vb, Ra, Rb, out collision);
        ...
        public static int FindLineCircleIntersections(Vector2 center, float radius, Vector2 from, Vector2 to, out Vector2 intersection1, out Vector2 intersection2)
        {
            var cx = center.X;
            var cy = center.Y;
            ...
            C = (from.X - cx) * (from.X - cx) + (from.Y - cy) * (from.Y - cy) - radius * radius;
            ...
            if (det == 0)
            {
                // One solution.
                t = -B / (2 * A);
                intersection1 = new Vector2(from.X + t * dx, from.Y + t * dy);
                intersection2 = new Vector2(float.NaN, float.NaN);
            ...
            else
            {
                ...
                intersection1 = new Vector2(from.X + t * dx, from.Y + t * dy);
                t = (float) ((-B - Math.Sqrt(det)) / (2 * A));
                intersection2 = new Vector2(from.X + t * dx, from.Y + t * dy);
